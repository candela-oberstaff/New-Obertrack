      ...
      <div className={styles['modal-content']} onClick={e => e.stopPropagation()}>
        <div className={styles['modal-header']}>
          {isEditing ? (
            ...
            <h2>Configuración de {selectedChannel.type === 'private' ? '🔒' : '#'}{selectedChannel.name}</h2>
          )}
          <button className={styles['close-btn']} onClick={onClose}>×</button>
        </div>

        {isEditing ? (
          <div className={styles['edit-form']} style={{ display: 'flex', flexDirection: 'column', gap: '12px', margin: '16px 0' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '12px' }}>
              ...
              <button 
                ...
                style={{ padding: '8px 16px', background: '#1d1c1d', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
              >
